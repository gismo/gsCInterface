...
GISMO_EXPORT gsCQuadRule* gsQuadrature_get(gsCBasis * basis, gsCOptionList* options, int fixDir)
{
    ...
    auto * options_ptr = reinterpret_cast<gismo::gsOptionList*>(options);
    return reinterpret_cast<gsCQuadRule*>(gismo::gsQuadrature::getPtr(*basis_ptr, *options_ptr, fixDir).release());
}
...
{
    delete reinterpret_cast<gismo::gsQuadRule<double>*>(qr);
}

GISMO_EXPORT int gsQuadRule_numNodes(gsCQuadRule * qr)
{
    return reinterpret_cast<gismo::gsQuadRule<double>*>(qr)->numNodes();
}

GISMO_EXPORT int gsQuadRule_dim(gsCQuadRule * qr)
{
    return reinterpret_cast<gismo::gsQuadRule<double>*>(qr)->dim();
}

GISMO_EXPORT void gsQuadRule_mapTo(gsCQuadRule* qr, gsCVector* lower, gsCVector* upper, gsCMatrix* nodes, gsCVector* weights)
{
    reinterpret_cast<gismo::gsQuadRule<double>*>(qr)->mapTo(*RICAST_V(lower), *RICAST_V(upper), *RICAST_M(nodes), *RICAST_V(weights));
}

GISMO_EXPORT void gsQuadRule_mapToScalar(gsCQuadRule* qr, double startVal, double endVal, gsCMatrix* nodes, gsCVector* weights)
{
    reinterpret_cast<gismo::gsQuadRule<double>*>(qr)->mapTo(startVal, endVal, *RICAST_M(nodes), *RICAST_V(weights));
}
